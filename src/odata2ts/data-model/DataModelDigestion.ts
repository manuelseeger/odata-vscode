import { MappedConverterChains } from "../stubs";
import type { DigestionOptions } from "../FactoryFunctionModel";
import {
...
> {
    ...
    protected constructor(
        ...
        this.nameValidator = options.bundledFileGeneration
            ? new NameClashValidator(options)
            : new NoopValidator();
    ...
    private digestEntityTypesAndOperations() {
        ...
        this.schemas.forEach((schema) => {
            this.analyzeModelUsage(
                schema.EntityContainer?.length ? schema.EntityContainer[0] : undefined,
            );
        });
    }
    ...
    private getBaseModel(
        entityConfig:
            | WithoutName<EntityTypeGenerationOptions | ComplexTypeGenerationOptions>
            | undefined,
        model: ComplexType,
        namespace: string,
        name: string,
        ...
        if (model.$.BaseType) {
            ...
            const [baseName, basePrefix] = this.namingHelper.getNameAndServicePrefix(
                model.$.BaseType,
            );
            const baseConfig =
    ...
    private addEnum(namespace: NamespaceWithAlias, models: Array<EnumType> | undefined) {
        ...
        for (const et of models) {
            ...
            const enumName = this.nameValidator.addEnumType(
                fqName,
                config?.mappedName || odataName,
            );
    ...
    private addComplexType(namespace: NamespaceWithAlias, models: Array<ComplexType> | undefined) {
        ...
        for (const model of models) {
            ...
            const name = this.nameValidator.addComplexType(
                fqName,
                config?.mappedName || model.$.Name,
            );
    ...
    private addEntityType(namespace: NamespaceWithAlias, models: Array<ET> | undefined) {
        ...
        for (const model of models) {
            ...
            const fqName = withNamespace(namespace[0], model.$.Name);
            const name = this.nameValidator.addEntityType(
                fqName,
                entityConfig?.mappedName || model.$.Name,
            );
    ...
    private analyze(fqModelName: string) {
        ...
        const model =
            this.dataModel.getEntityType(fqModelName) ?? this.dataModel.getComplexType(fqModelName);
    ...
    private collectBaseClassPropsAndKeys(
        ...
        return model.baseClasses.reduce(
            ([props, keys, attributes], bc) => {
                const baseModel =
                    this.dataModel.getEntityType(bc) || this.dataModel.getComplexType(bc);
                if (!baseModel) {
                ...
                if (baseModel.baseClasses.length) {
                    const [parentProps, parentKeys, parentAttributes] =
                        this.collectBaseClassPropsAndKeys(baseModel, visitedModels);
    ...
    protected mapProp = (
        ...
        let result: Pick<
            PropertyModel,
            "dataType" | "type" | "typeModule" | "qPath" | "qParam" | "qObject" | "converters"
        >;
        ...
        if (this.namingHelper.includesServicePrefix(odataDataType)) {
            ...
            const [dataTypeName, dataTypePrefix] =
                this.namingHelper.getNameAndServicePrefix(odataDataType);
            ...
            if (modelType === DataTypes.EnumType) {
                const enumConfig = this.serviceConfigHelper.findEnumTypeConfig(
                    dataTypeNamespace,
                    dataTypeName,
                );
                result = {
            ...
            // handling of complex & entity types
                ...
                const entityConfig =
                    modelType === DataTypes.ComplexType
                        ? this.serviceConfigHelper.findComplexTypeConfig(
                              dataTypeNamespace,
                              dataTypeName,
                          )
                        : this.serviceConfigHelper.findEntityTypeConfig(
                              dataTypeNamespace,
                              dataTypeName,
                          );
        ...
        // OData built-in data types
            ...
            const {
                ...
                toModule: typeModule,
                converters,
            } = this.dataModel.getConverter(odataDataType) || {};

            const type = !to
                ? outputType
                : to.startsWith(Digester.EDM_PREFIX)
                  ? this.mapODataType(to).outputType
                  : to;
        ...
        return {
            ...
            managed:
                typeof entityPropConfig?.managed !== "undefined"
                    ? entityPropConfig.managed
                    : configProp?.managed,
            ...result,
        };
    };
