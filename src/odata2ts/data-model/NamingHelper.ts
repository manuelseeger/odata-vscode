...
import {
    FileNamingStrategyOption,
    NameSettings,
    NamingStrategies,
    StandardNamingOptions,
} from "../NamingModel";
...
export class NamingHelper {
    ...
    private namingFunction(strategy: NamingStrategies | undefined) {
        ...
        return (value: string, options?: StandardNamingOptions) => {
            ...
            let result = strategyFn(
                (prefix ? prefix + "_" : "") + value + (suffix ? "_" + suffix : ""),
            );
    ...
    public getQFunctionName(operationName: string, boundEntity?: string) {
        ...
        const result = this.getName(
            operationName,
            this.getQObjectNamingStrategy(),
            opts?.function || opts,
        );
        const name = this.getName(
            result,
            this.getQObjectNamingStrategy(),
            this.options.queryObjects,
        );
        return this.getPrefixedName(name, boundEntity);
    }
    ...
    public getMainServiceName() {
        ...
        const strategy = this.namingFunction(
            opts?.main?.namingStrategy ??
                (opts?.main?.applyServiceNaming ? opts.namingStrategy : undefined),
        );
    ...
    public getFunctionName(operationName: string) {
        ...
        return this.getName(
            operationName,
            this.getOperationNamingStrategy(),
            opts?.function || opts,
        );
    }
