...
export class DataModel {
    ...
    private container: EntityContainerModel = {
        entitySets: {},
        singletons: {},
        functions: {},
        actions: {},
    ...
    private retrieveType<T>(fqName: string, haystack: Map<string, T>): T | undefined {
        return (
            haystack.get(fqName) ||
            (this.aliases[fqName] ? haystack.get(this.aliases[fqName]) : undefined)
        );
    }
    ...
    public getEntityTypes() {
        const ets = [...this.models.values()].filter(
            (m): m is EntityType => m.dataType === DataTypes.ModelType,
        );
    ...
    public getComplexTypes() {
        const types = [...this.models.values()].filter(
            (m): m is ComplexType => m.dataType === DataTypes.ComplexType,
        );
    ...
    public getEnums() {
        return [...this.models.values()].filter(
            (m): m is EnumType => m.dataType === DataTypes.EnumType,
        );
    }
    ...
    public addUnboundOperationType(namespace: string, operationType: OperationType) {
        ...
        const existingFn = isFunction
            ? this.unboundOperationTypes.get(operationType.fqName)
            : undefined;
        if (existingFn) {
            const params = operationType.parameters;
            existingFn.overrides
                ? existingFn.overrides.push(params)
                : (existingFn.overrides = [params]);
            return;
        }
    ...
    public addBoundOperationType(
        namespace: string,
        bindingProp: PropertyModel,
        operationType: OperationType,
    ) {
        const fqEntityType = bindingProp.fqType;
        const store = bindingProp.isCollection
            ? this.entityCollectionBoundOperationTypes
            : this.entityBoundOperationTypes;
        ...
        if (boundOps) {
            ...
            const isFunction = operationType.type === OperationTypes.Function;
            const existingFn = isFunction
                ? boundOps.find((bo) => bo.fqName === operationType.fqName)
                : undefined;
            if (existingFn) {
                const params = operationType.parameters;
                existingFn.overrides
    ...
    public getEntitySetOperations(fqEntityName: string): Array<OperationType> {
        const operations = this.retrieveType(
            fqEntityName,
            this.entityCollectionBoundOperationTypes,
        );
        return operations || [];
    ...
    public getAllEntityOperations(fqEntityName: string): Array<OperationType> {
        return [
            ...this.getEntityTypeOperations(fqEntityName),
            ...this.getEntitySetOperations(fqEntityName),
        ];
    }
    ...
    private sortModelsByInheritance<Type extends Omit<ComplexType, "dataType">>(
        models: Array<Type>,
    ): Array<Type> {
