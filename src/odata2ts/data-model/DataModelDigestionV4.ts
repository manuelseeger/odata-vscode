import { loadConverters, MappedConverterChains } from "../stubs";
    ...
    protected digestEntityContainer(schema: SchemaV4) {
        if (schema.EntityContainer && schema.EntityContainer.length) {
            ...
            container.ActionImport?.forEach((actionImport) => {
                ...
                const opConfig = this.serviceConfigHelper.findOperationImportConfig(
                    ecName,
                    odataName,
                );
                const opName = this.nameValidator.addOperationImportType(
                    fqName,
                    opConfig?.mappedName || odataName,
                );
            ...
            container.Singleton?.forEach((singleton) => {
                ...
                const fqName = withNamespace(ecName, odataName);
                const singletonConfig = this.serviceConfigHelper.findOperationImportConfig(
                    ecName,
                    odataName,
                );
            ...
            container.EntitySet?.forEach((entitySet) => {
                ...
                const fqName = withNamespace(ecName, odataName);
                const config = this.serviceConfigHelper.findOperationImportConfig(
                    ecName,
                    odataName,
                );
                const name = this.nameValidator.addEntitySet(
                    fqName,
    ...
    private addOperations(
        ...
        operations.forEach((op) => {
            ...
            if (isBound && !params.length) {
                throw new Error(
                    `IllegalState: Operation '${odataName}' is bound, but has no parameters!`,
                );
            }

            const bindingProp = isBound ? params.shift() : undefined;
            const bindingEntityName = bindingProp
                ? this.dataModel.getModel(bindingProp!.fqType)?.name
                : undefined;

            const opName = bindingEntityName
                ? this.nameValidator.addBoundOperationType(
                      bindingEntityName,
                      fqName,
                      opConfig?.mappedName || odataName,
                      type,
                  )
                : this.nameValidator.addUnboundOperationType(
                      fqName,
                      opConfig?.mappedName || odataName,
                      type,
                  );
            ...
            const opType: OperationType = {
                ...
                name,
                qName,
                paramsModelName: this.namingHelper.getOperationParamsModelName(
                    opName,
                    bindingEntityName,
                ),
                type,
