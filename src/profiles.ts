...
export class ProfileTreeProvider implements vscode.TreeDataProvider<ProfileItem> {
            ...
            async (message) => {
                if (message.command === 'saveProfile') {
                    ...
                    if (fileUri && fileUri.length > 0) {
                        panel.webview.postMessage({ command: 'fileSelected', inputName: type, filePath: fileUri[0].path });
                    }
                }
            },
            undefined,
    ...
    private _getWebViewContent(webview: vscode.Webview, profile?: Profile): string {
        ...
        const stylesUriList = styles.map(style => webview.asWebviewUri(vscode.Uri.file(path.join(this.context.extensionPath, style))));
        const linkTags = stylesUriList.map(uri => `<link href="${uri}" rel="stylesheet" />`).join('\n');
          ...
          <body>
            ...
            <form id="profileForm">
              <div style="display: flex; gap: 20px;">
                ...
                ? Object.keys(profile.headers).map(key =>
                    `<div>
