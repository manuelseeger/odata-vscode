...
import { ODataVersion } from './odata2ts/data-model/DataTypeModel';
...
const odataMethods = {
    "V2": [
        ...
        { name: "substring", doc: "Extracts a substring from a string starting at a specified index." },
    ...
    "V4": [
        ...
        { name: "totaloffsetminutes", doc: "Calculates the total minutes of the time zone offset." },
...
const odataSystemQueryOptions = {
    ...
    "V4": [
        { name: "$apply", doc: "Applies aggregations or transformations to the resource collection." },
...
export class ODataDefaultCompletionItemProvider implements vscode.CompletionItemProvider {
    ...
    async provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext): Promise<vscode.CompletionList<vscode.CompletionItem> | null | undefined> {
        ...
        if (document.getWordRangeAtPosition(position, /\$inlinecount=.*/)) {
            return new vscode.CompletionList([new vscode.CompletionItem("allpages", vscode.CompletionItemKind.EnumMember)]);
        }
        ...
        const profile = vscode.workspace.getConfiguration("odata").get("selectedProfile") as Profile;
